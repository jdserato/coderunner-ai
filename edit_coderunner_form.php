...
class qtype_coderunner_edit_form extends question_edit_form {
    ...
    public function definition_inner($mform) {
        $this->add_generate_ai_field($mform);
        echo "WORKING";
     ...
     */
    protected function add_generate_ai_field($mform) {
        global $CFG;
        $qtype = question_bank::get_qtype('coderunner');
        // Insert the AI GENERATION section to allow file uploads.
        $mform->addElement('header', 'generateAIoptions', get_string('generateAIoptions', 'qtype_coderunner'));
        $mform->setExpanded('generateAIoptions', 1);

        // $mform->addElement('select', 'generateAI',
        //         get_string('generateAI', 'qtype_coderunner'), $qtype->generateai_options());
        // $mform->setDefault('generateAI', 0);

        // Feedback control (a group with only one element).
        $genaielements = array();
        $genaivalues = array(
            constants::GENAI_ARITHMETIC=> get_string('genai_arithmetic', 'qtype_coderunner'),
            constants::GENAI_SELECTION => get_string('genai_selection', 'qtype_coderunner'),
            constants::GENAI_ITERATIVE => get_string('genai_iterative', 'qtype_coderunner'),
        );

        $genaielements[] = $mform->createElement('select', 'generateAI', null, $genaivalues);
        $mform->addElement('group', 'generateAI',
                get_string('generateAI', 'qtype_coderunner'), $genaielements, null, false);

        $mform->addHelpButton('generateAI', 'generateAI', 'qtype_coderunner');
        $mform->registerNoSubmitButton('generateAI_button');
        $mform->addElement('submit', 'generateAI_button',
                get_string('generateAI_button', 'qtype_coderunner'));
    }
    ...
    public function data_preprocessing($question) {
        ...
        $open_ai_key = get_config('qtype_coderunner', 'chatgpt_apikey');
        ...
        if (isset($question->options->testcases)) {
            ...
            // // Get Content
            // echo("HERE I AM BITCHH");
            // // echo($d);
            // echo($d->choices[0]->message->content);
            // echo("HERE I AM BITCH3");
            // // throw new Exception("Wa lang lols debuggerszx");
            // $meow = $d->choices[0]->message->content; // intentional error
            // $complete = $open_ai->chat([
            //    'model' => 'gpt-3.5-turbo',
            //    'messages' => [
            //        [
            //            "role" => "user",
            //            "content" => "Give one easy programming problem involving ".$type
            //        ],
            //    ],
            //    'temperature' => 1.0,
            //    'max_tokens' => 4000,
            ...
            // ]);
            // $d = json_decode($complete);
            // $question->questiontext['text'] = $d->choices[0]->message->content;
            // $question->questiontext = $d->choices[0]->message->content;
            foreach ($question->options->testcases as $tc) {
                // $question->testcode[] = $this->newline_hack($d->choices[0]->message->content);
        ...
        $data = $this->get_submitted_data();
        // echo("SUBMIT VALS");
        //  print_r($this->_form->_submitValues);
        //     foreach (array_keys($this->_form->_elements) as $key) {
        //         if ($this->_form->_elements[$key]->isFrozen() && !$this->_form->_elements[$key]->_persistantFreeze) {
        //             echo("FROZEN AND NOT PERSISTANT ");
        //             $value = '';
        //             if (isset($this->_form->_elements[$key]->_attributes['name'])) {
        //                 echo("VALUE SET".$this->_form->_elements[$key]);
        //                 $varname = $this->_form->_elements[$key]->_attributes['name'];
        //                 // If we have a default value then export it.
        //                 if (isset($this->_form->_defaultValues[$varname])) {
        //                     $value = $this->_form->prepare_fixed_value($varname, $this->_defaultValues[$varname]);
        //                 } 
        //             } else {
        //                 echo("VALUE UNSET".$this->_form->_elements[$key]);
        //             }
        //         } else {
        //             echo("UNFROZEN ");
        //             $value = $this->_form->_elements[$key]->exportValue($this->_form->_submitValues, true);
        //         }
        //         if (is_array($value)) {
        //             print_r($value);
        //         }
        ...
        if (!empty($data->generateAI_button)) {
            ...
            // $data["updatebutton"] = 1;
            // $this->setConstant("updatebutton", 1);
            // $this->_form->_elements["updatebutton"] = 1;
            // echo("\n\nALLELEM\n");
            // print_r($this->_form->_elements->_attributes['name']);
            // echo("\n\nDATA\n");
            // print_r($data);
            // $this->_form->_submitValues["updatebutton"] = "Save changes and continue editing";
            $data = $this->get_submitted_data();
        }
        // if (!empty($data->updatebutton)) {
        //     throw new Exception("Update button clicked");
        // }
        // if (!empty($data->submitbutton)) {
        //     throw new Exception("Submit button clicked");
        // }
        if (!empty($data->generateAI_button)) {
            // if ($question->options) {
            ...
            // }
            $mform = $this->_form;
            $qtext = $mform->getElement('answerpreload');
            $qtext->setValue("SAMPLE sampl samp sam sa s");
            $generated = true;
            // echo("QUESTION before");
            // print_r($question);
            // echo("HERE I AM testing");
            // print_r($data);
            if ($data->generateAI == constants::GENAI_ARITHMETIC) {
                $type = "Arithmetic";
            } else if ($data->generateAI == constants::GENAI_ITERATIVE) {
                $type = "Iteration";
            } else if ($data->generateAI == constants::GENAI_SELECTION) {
                $type = "Selection";
            }
            $complete = $open_ai->chat([
               'model' => 'gpt-3.5-turbo',
               'messages' => [
                   [
                       "role" => "user",
                       "content" => "Give one programming problem involving ".$type
                   ],
               ],
               'temperature' => 1.0,
               'max_tokens' => 4000,
            ...
            $question->questiontext['text'] = "TESTING LANG NAMAN NOHH";
            ...
            echo("NA SET BA ".isset($data->generateAI_button));
        }
    ...
    private function make_questiontype_panel($mform) {
        ...
        $twigelements[] = $mform->createElement('select', 'templateparamslang',
                get_string('templateparamslang', 'qtype_coderunner'), $templateparamlangs);
    ...
    public function definition_after_data() {
        ...
        $mform = $this->_form;
        $qtext = $mform->getElement('answerpreload');
        $form_answer = $mform->getElement('answer');
        $form_gextra = $mform->getElement('globalextra');
        $form_question = $mform->getElement('questiontext');
        $open_ai_key = get_config('qtype_coderunner', 'chatgpt_apikey');
        $open_ai = new OpenAi($open_ai_key);
        ...
        if (!empty($data->generateAI_button)) {
            ...
            // $this->question->questiontext = $d->choices[0]->message->content;
            // $this->question->questiontext = array();
            $this->question->questiontext = "TESTING LANG NAMAN NOHH";
            // $this->set_data($this->question);
            // $this->question->questiontext['format'] = empty($this->question->questiontextformat) ?
                    // editors_get_preferred_format() : $this->question->questiontextformat;
            echo("MAO NA NI!!!");
            // echo($this->question->questiontext);
            ...
            foreach ($this->question->options->testcases as $tc) {
                $this->question->testcode[] = $this->newline_hack($d->choices[0]->message->content);
                // $this->question->testcode[] = $this->newline_hack($tc->testcode);
                $this->question->testtype[] = $tc->testtype;
                $this->question->stdin[] = $this->newline_hack($tc->stdin);
                $this->question->expected[] = $this->newline_hack($tc->expected);
                $this->question->extra[] = $this->newline_hack($tc->extra);
                $this->question->useasexample[] = $tc->useasexample;
                $this->question->display[] = $tc->display;
                $this->question->hiderestiffail[] = $tc->hiderestiffail;
                $this->question->mark[] = sprintf("%.3f", $tc->mark);
            }
            // $url = new moodle_url('/question/bank/editquestion/question.php');
            // $url->param('id', $this->question->id);
            // $url->remove_params('makecopy');
            // redirect($url);
            // throw new Exception("Wa lang lols debuggerszx");
            // $this->display();
